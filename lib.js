let lib = ( () => {
    ...
    let exp = (item, title) => {
        ...
        item.forEach(it = (it) =>{
            aux= aux+`<div class="div-cv">
                ...
                <p class="message-medium">${it['details']}</p>
            </div>`
        })
    ...
    let builder = () => {
        ...
        let we = exp(dt['professional'], "Working Experience")
        let se = exp(dt['studies'], "Education")
        let sk = sks(dt['skills'], "Skills")
        document.getElementById('article').innerHTML= we+se+sk
    }
    return {
        build: () => builder()
    };
